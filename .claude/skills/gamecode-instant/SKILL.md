...
`/gamecode` with its review gate removed. Same pipeline, same sub-skills, same
rules — the difference is that nothing waits for the user until the work is
built.

```
/gamecode-instant <idea>
  → brainstorming → branch → spec → writing-plans
  → the Summary (sent, never awaited)
  → executing-plans → report → "Ship it?"
```
...
|---|---|
| §4 Plan Summary, then wait | the same message, sent as a notice — the run continues in the same turn (§R) |
| §5 amendment folded in after the yes | there is no yes — see **Interrupts** (§S) |
| "Ship it?" | unchanged, and **not waivable** (§S) |

## R. The Summary

`/gamecode` §4 verbatim — same parts, same 200-word cap, same repo-relative
paths — with two changes:

- **`Paling perlu dilihat` carries more weight here**, because nobody is going
  to answer it. Put the guess most likely to be wrong there anyway: it is what
  the user scans for when they come back, and what tells them whether to read
  the branch or bin it.
