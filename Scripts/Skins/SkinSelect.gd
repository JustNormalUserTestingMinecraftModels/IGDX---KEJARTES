...
@onready var _track: HBoxContainer = %Track
...
## Track.position.x when the current drag began, and the global x of the
## press, so travel is measured against the press rather than the last
## motion event.
var _drag_start_track_x: float = 0.0
...
func _apply_card_states() -> void:
	for i in _track.get_child_count():
		var card := _track.get_child(i) as SkinCard
		if card != null:
			card.set_selected(i == _skin_index)


## One card's width plus the track's separation -- how far the track moves
## between neighbouring cards. Read off the first card rather than
## hardcoded, so the card scene stays the single source of its own size.
func _pitch() -> float:
	if _track.get_child_count() == 0:
		return 1.0
	var card := _track.get_child(0) as Control
	var sep := float(_track.get_theme_constant("separation"))
	return card.get_combined_minimum_size().x + sep


## Moves the track so card `index` sits in the middle of the carousel.
func _slide_to(index: int, animated: bool) -> void:
	if _track.get_child_count() == 0:
		return
	var card := _track.get_child(0) as Control
	var card_w := card.get_combined_minimum_size().x
	var target := _carousel.size.x * 0.5 - card_w * 0.5 - _pitch() * float(index)
	...
	_slide_tween.tween_property(_track, "position:x", target, slide_time)
...
func _begin_drag(at_x: float) -> void:
	if is_instance_valid(_slide_tween) and _slide_tween.is_valid():
		_slide_tween.kill()
	_dragging = true
	_drag_start_track_x = _track.position.x
	_drag_start_x = at_x
	_last_x = at_x
...
func _update_drag(at_x: float) -> void:
	_track.position.x = _drag_start_track_x + (at_x - _drag_start_x)
...
func _end_drag() -> void:
	if not _dragging:
		return
	_dragging = false
	var travel := _track.position.x - _drag_start_track_x
	select_skin(SkinCard.settle_index(_skin_index, travel, _release_velocity,
		_pitch(), _track.get_child_count()))
