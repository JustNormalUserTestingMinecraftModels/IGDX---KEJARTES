...
func _ready() -> void:
	...
	# Starts inert: the same tap that opened this popup (via a trait
	# badge's pressed signal) would otherwise also land on this
	# full-screen scrim in the same frame and immediately close what it
	# just opened. open() re-enables input once the reveal finishes.
